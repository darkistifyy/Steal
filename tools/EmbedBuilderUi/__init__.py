...
from tools.EmbedBuilder import *
...
class EmbedEditor(discord.ui.View):
	...
	@discord.ui.button(label='Create', row=2, style=ButtonStyle.red)
	async def send(self, interaction: BotInteraction, button: discord.ui.Button[Self]):
		...
		embed_code = EmbedBuilder.copy_embed(self, interaction.message)
		await interaction.message.edit(content=f"```{embed_code}```", view=None, embed=None) 
