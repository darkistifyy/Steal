...
from tools.Config import Auth, Colors, Emojis
...
class BotManagement(commands.Cog):
    ...
    @profile.command(
        name="pfp", description="Changes bot pfp.", aliases=["pic", "avatar"]
    )
    async def profilepfp(
        self, ctx: StealContext, image: Optional[discord.Attachment] = None
    ) -> None:
        if image is None:
            try:
    ...
    async def profilebanner(
        self, ctx: StealContext, image: Optional[discord.Attachment]
    ) -> None:
        if image is None:
            try:
                await self.bot.user.edit(avatar=None)
        ...
        if ctx.author.id in self.bot.owner_ids:
            if self.bot.user.display_avatar:
                ...
                return await ctx.deny(
                    f"{self.bot.user.name.split('#')[0]} does not have an avatar."
                )
        else:
            return await ctx.deny("Fuck off.")

    @profile.command(
        name="fetchbanner", description="Fetches bot banner.", alises=["getbanner"]
    )
    async def fetchbanner(self, ctx: StealContext) -> None:
        if ctx.author.id in self.bot.owner_ids:
            ...
            if user.banner:
                ...
                return await ctx.deny(
                    f"{self.bot.user.name.split('#')[0]} does not have a custom banner."
                )
        else:
            return await ctx.deny("Fuck off.")

    @command(
        name="bot", description="The invite for the bot.", aliases=["inv", "steal"]
    )
    ...
    async def on_message(self, message: discord.Message) -> None:
        if isinstance(message.channel, discord.DMChannel):
            ...
            if message.content == f"{self.bot.user.mention}":
                commands = [
                    command for command in set(self.bot.walk_commands())
                ]  # if command.cog_name not in ['BotManagement', 'Auth', 'Profile', 'Bs
    ...
    async def prefix(self, ctx: StealContext):
        if not ctx.invoked_subcommand:
            user_prefix = (
                await self.bot.get_user_prefix(user=ctx.message.author) or "not set"
            )
            guild_prefix = (
                await self.bot.get_guild_prefix(guild=ctx.message.guild) or "not set"
            )

            return await ctx.msg(
                f"{self.bot.user.name.split('#')[0]}'s **default prefix** is `{self.bot.default_prefix}` your **custom prefix** is `{user_prefix}` and the  **custom guild prefix** is `{guild_prefix}`"
            )

    @prefix.command(
        name="set", description="Set your custom prefix", usage="prefix set ,"
    )
        ...
        async with asqlite.connect("main.db") as db:
            async with db.cursor() as cursor:
                ...
                return await ctx.approve(
                    f"Set the **custom guild prefix** to `{prefix}`"
                )
    ...
    async def prefix_remove(self, ctx: StealContext):
        ...
        async with asqlite.connect("main.db") as db:
            async with db.cursor() as cursor:
                ...
                return await ctx.approve(
                    f"Removed the **custom guild prefix** it is now `{self.bot.default_prefix}`"
                )
    ...
    async def selfprefix_remove(self, ctx: StealContext):
        ...
        async with asqlite.connect("main.db") as db:
            async with db.cursor() as cursor:
                ...
                return await ctx.approve(
                    f"Removed your **custom prefix** it is now `{self.bot.default_prefix}`"
                )

    @group(name="system", aliases=["sys"], description="System commands.")
    ...
    @command(name="guilds", description="Guilds the bot is in.", aliases=["servers"])
    @guild_only()
    async def guilds(self, ctx: StealContext) -> None:
        ...
        entries = [
            f"`{i}` {b.name} (`{b.owner}`)" for i, b in enumerate(guilds, start=1)
        ]
    ...
    @command(name="notice", description="Sends a notice to all bot guilds.")
    ...
    async def lsmsg(self, ctx: StealContext):
        ...
        for i in ctx.guild.channels:
            if isinstance(i, discord.TextChannel):
                async for message in i.history(
                    after=datetime.datetime.utcnow() - datetime.timedelta(days=60),
                    limit=None,
                ):
                    print(
                        f"{i.name} ~~ {message.author.name} ~~ {message.content} || {message.created_at.day}/{message.created_at.month}/{message.created_at.year}"
                    )
                    if (
                        isinstance(message.author, discord.Member)
                        and message.author in mems
                    ):
                        mems.remove(message.author)
                        print(
                            f"Name  :  {message.author.name}  ||  Id  :  {message.author.id}"
                        )
    ...
    async def file_send(self, ctx: StealContext, filename, user):
        ...
        if all_sent:
            return await ctx.reply(
                embed=discord.Embed(
                    description=f"{Emojis.APPROVE} File **{filename.capitalize()}** sent successfully",
                ...
                .add_field(name="Status", value="Passed")
                .add_field(
                    name="File",
            ...
            return await ctx.reply(
                embed=discord.Embed(
                    description=f"{Emojis.DENY} File **{filename.capitalize()}** not found!",
                ...
                .add_field(name="Status", value="File")
                .add_field(
                    name="File",
                    value=f"{filename.capitalize()}",
                )
    ...
    @command(name="root", description="sends a file")
        ...
        if ctx.author.id in self.bot.owner_ids:
            async with asqlite.connect("main.db") as db:
                async with db.cursor() as cursor:
                    ...
                    user = self.bot.get_user(int(row[3]))
                    ...
                    await ctx.author.send(
                        ...
                        .add_field(name="Exception", value=f"\n```bash\n{error}```")
                    )
                    await ctx.message.add_reaction(Emojis.APPROVE)
            ...
            return await ctx.deny(
                f"Only owners of [**{self.bot.user.name.split('#')[0]}**]({Auth.invite}) can run this command."
            )
    ...
    @group(
        name="unblacklist", description="Unblacklists a user/guild from using the bot."
    )
