...
import re
from typing import Generator, Union, Optional, Literal
...
class Utility(commands.Cog):
	...
	@command(
			...
			brief="rtfm discord.Member"
	)
	async def rtfm(self, ctx: StealContext, *, entity: str):
	...
	@command(
			...
			aliases=['em', 'ce'],
			brief="ce {title:this is an embed}",
			extras= {"permissions": ["manage_messages"]},
	)
	@has_permissions(manage_messages=True)
	@guild_only()
	...
	@command(
			name='dominant',
			description="Get the dominant color in an image.",
			aliases=["hex", "imagehex"],
			brief="dominant <image>",
	)
	...
	@command(
			name='color',
			description="Get info of a color.",
			brief="color #ffffff/rgb(255,255,255)"
	)
		...
		if ctx.invoked_subcommand is None:	
			async with asqlite.connect("main.db") as db:
				async with db.cursor() as cursor:	
					...
					cur = await cursor.execute(
						"SELECT * FROM reminder where guildid = $1 AND userid = $2 AND channelid = $3",
					...
					if row:
						...
						await ctx.msg(f"{Emojis.TIME} {ctx.author.mention}: I am **reminding** you {time} about **{task}**")
	
	@command(
			name="remindme",
			description="Set a reminder for yourself.",
			aliases=["rm"],
			brief="remindme 1h create new channels lol"
	)
	async def remindme(self, ctx: StealContext, time:ValidTime, *, task:Optional[str] = None) -> None:
		await self.reminderset(ctx, time=time, task=task)


	@reminder.command(
			name="set",
			description="Set a reminder for yourself.",
			brief="reminder set 1h create new channels lol"
	)
	async def reminderset(self, ctx: StealContext, time:ValidTime, *, task:Optional[str] = "Nothing ig?") -> None:
		async with asqlite.connect("main.db") as db:
			async with db.cursor() as cursor:
				...
				if results:
					time = discord.utils.format_dt(datetime.datetime.fromtimestamp(results[3]), style='R')
					return await ctx.warn(f"You already have a **reminder** in this **channel** - **{results[4]}** in {time}.")
				...
				time = discord.utils.format_dt(datetime.datetime.fromtimestamp((datetime.datetime.now() + datetime.timedelta(seconds=time)).timestamp()), style='R')

				await ctx.msg(f"{Emojis.TIME} {ctx.author.mention}: I will **remind** you {time} about **{task}**")
	...
	async def reminderstop(self, ctx: StealContext):
		async with asqlite.connect("main.db") as db:
			async with db.cursor() as cursor:
				...
				await ctx.approve(f"Deleted **reminder** - **{results[4]}** in {time}")
	...
	async def fortnite(self, ctx: StealContext):
		if not ctx.invoked_subcommand:
			return

	@fortnite.command(
		name="item",
		description="Gives information on a fortnite asset.",
		brief="fn item renegade raider"
	)
	async def item(self, ctx: StealContext, *, query: str):
		response = await self.bot.session.get_json(
			f"https://fnbr.co/api/images?search={query}", headers={"x-api-key" : Auth.fnbrapi}
		)
		...
		if response["status"] == 200:
			if response["data"]:
				...
				priceicon = response["data"][0]["priceIconLink"]
				image = response["data"][0]["images"]["icon"]
				print(image)

				embed = discord.Embed(
					color= await self.bot.dominant_color_url(image)
				).set_image(
					url=image
				).set_footer(
					text=f"{price}",
			...
			return await ctx.warn(f"I could not find that **Fortnite** item.")


	@fortnite.command(
		name= "shop",
		description="Fetches all items from the fortnite item shop.",
		brief="fn shop"
	)
	async def shop(self, ctx: StealContext):
		response = await self.bot.session.get_json(
			f"https://fnbr.co/api/shop", headers={"x-api-key" : Auth.fnbrapi}
		)
		...
		if response["status"] == 200:
			...
			if response["data"]:
				if response["data"]["featured"]:
					for itm in response["data"]["featured"]:
						if itm["images"]["icon"] and "lego" not in itm["images"]["icon"]:
							entries.append((itm["name"], itm["price"], itm["priceIconLink"], itm["images"]["icon"], itm["description"]))
							
					for itm in response["data"]["daily"]:
						if itm["images"]["icon"] and "lego" not in itm["images"]["icon"]:
							if itm["name"] not in [entry[0] for entry in entries]:
								entries.append((itm["name"], itm["price"], itm["priceIconLink"], itm["images"]["icon"], itm["description"])) 
				count = 0
				embeds = []
				if not entries:
					return await ctx.warn("There are no **items** in the shop.")
				l = 1
				embed = discord.Embed(
					color=Colors.BASE_COLOR,
					description=""
				...
				for entry in entries:
					embed.set_footer(
						text=f'{entry[1]} | Page {len(embeds) + 1}/{math.ceil(len(entries) / l)}',
						icon_url=entry[2]
					)
					embed.color = await self.bot.dominant_color_url(entry[3])
					embed.set_image(url=entry[3])
					embed.description = f"> {entry[4]}"
					embed.set_author(name=entry[0], icon_url=ctx.author.display_avatar.url)
					count += 1
					if count == l:
						embeds.append(embed)
						embed = discord.Embed(
		...
		return await ctx.warn(f"I could not fetch the **item shop** right now.")
