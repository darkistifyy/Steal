...
class PatchedInteraction(Interaction):
    ...
    async def approve(self, message: str, url: str=None) -> discord.Message:
        if url is None:
            return await self.response.send_message(embed=discord.Embed(description=f'{Emojis.APPROVE} {self.user.mention}: {message}', color=Colors.BASE_COLOR), ephemeral=True)
