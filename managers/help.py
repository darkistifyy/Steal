...
class StealHelp(HelpCommand):
    def __init__(self):
        super().__init__()
        self.command_attrs = {"aliases": ["assist", "commands"]}
        self.verify_checks = True

    async def send_bot_help(
        self, mapping: Mapping[Cog, List[Command[Any, Callable[..., Any], Any]]]
    ) -> None:
        ...
        embed = (
            ...
            .set_author(
                ...
                if self.context.author.display_avatar
                else None,
            )
            .set_thumbnail(url=self.context.bot.user.display_avatar.url)
            .set_footer(text="Select a cog from the dropdown menu below")
            .add_field(
                name="Information",
                value="```ruby\n [] = optional, <> = required```",
                inline=False,
        ...
        view.add_item(
            HelpSelect(
                {
                    key.__cog_name__: key
                    for key, _ in mapping.items()
                    if key is not None
                    and key.__cog_name__
                    not in [
                        "BotManagement",
                        "Auth",
                        "Bs",
        ...
        return await self.context.send(embed=embed, view=view)
    ...
    async def send_command_help(self, command: Command) -> None:
        ...
        friendly_permissions = ", ".join(
            f"{perm.replace('_', ' ').title()}" for perm in permissions
        )
        ...
        return await self.context.send(
            ...
            .add_field(
                name="Aliases", value=(", ".join(command.aliases) or "N/A"), inline=True
            )
            .add_field(
                name="Parameters",
                value=(", ".join([parameter for parameter in command.params]) or "N/A"),
                inline=True,
            )
            .add_field(
                name="Usage",
                value=f">>> ```ruby\nSyntax: {prefix}{command.qualified_name} {' '.join([f'[{p.name}]' if not p.required else f'<{p.name}>' for p in command.clean_params.values()])}\nExample: {f'{prefix}{command.brief}' if command.brief else 'figure it out ig.'}```",
                inline=False,
            )
            .add_field(
                name="Permissions",
                value=f"{friendly_permissions if friendly_permissions else 'None'}",
            )
            .set_footer(
    ...
    async def send_group_help(self, group: Group) -> None:
        ...
        for command in group.commands:
            if isinstance(command, discord.ext.commands.Group):
                group_commands = [*group_commands, command, *command.commands]
        commands = [
            group,
            *[
                command
        ...
        return await self.context.paginate(
            [
                ...
                .add_field(
                    name="Aliases",
                    value=", ".join(command.aliases) or "N/A",
                    inline=True,
                )
                .add_field(
                    name="Parameters",
                    value=", ".join(
                        ...
                        else [parameter.name for parameter in group.commands]
                    )
                    or "N/A",
                    inline=True,
                )
                ...
                .set_footer(
                    text=f"Page {i + 1}/{len(commands)} ({len(commands)} entries) ∙ Module: {command.cog_name}",
                    icon_url=self.context.bot.user.display_avatar.url,
                )
...
class HelpSelect(discord.ui.Select):
        ...
        super().__init__(
            ...
            options=[
                discord.SelectOption(
                    label="Home", description="Beginning help page.", value="home"
                ),
                *[
                    discord.SelectOption(
                        label=key, description=category.description, value=key
                    )
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        if self.values[0] == "home":
            return await interaction.response.edit_message(
                embed=self.embed.add_field(
                    name="Information", value="> [] = optional, <> = required"
                ).add_field(
                    name="Support", value=f"[**Support server**]({Guild.INVITE})"
                )
            )
        ...
        commands = [
            command for command in list(category.walk_commands()) if not command.parent
        ]
        ...
        embed.set_footer(text=f"{len(commands)} commands.").set_author(
