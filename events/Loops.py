...
async def reminder_task(bot: Bot):
    async with asqlite.connect("main.db") as db:
        async with db.cursor() as cursor:
            ...
            cur = await cursor.execute("SELECT * FROM reminder")
...
async def giveaway_check(bot: Bot):
    async with asqlite.connect("main.db") as db:
        async with db.cursor() as cursor:
            ...
            cur = await cursor.execute("SELECT * FROM giveaways")
            ...
            for row in rows:
                try:
                    ...
                    if datetime.datetime.now().timestamp() > time and int(ended) != 1:
                        ...
                        if not entries:
                            if "GIVEAWAY ENDED" in message.content:
                                return
                            embed = discord.Embed(
                                title=f"{prize}",
                            ...
                            return await message.edit(
                                content="GIVEAWAY ENDED.", embed=embed
                            )
...
async def error_clear(bot: Bot):
    async with asqlite.connect("main.db") as db:
        async with db.cursor() as cursor:
            ...
            cur = await cursor.execute("SELECT * FROM errors")
